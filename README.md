...
- **[encap](bpf/encap/_encap.c)** is loaded once at `setup` but attached separately per ENI, to that ENI's veth-outer end (`add`). When the backend behind that veth replies, encap looks its 5-tuple up in the same `flow_state` map (swapped or literal, depending on whether the ENI is NAT/terminating or transparent — see `eni_mode` below), replays the cached outer header bytes completely unmodified — decap already pre-swapped its addressing — except for the fields that depend on this specific reply's own size (recomputed IPv4 checksum, total length), and redirects the packet back out the uplink toward GWLB. Three kinds of reply can't be matched by their own tuple and are handled specially:
  ...
  - **IPv6 Neighbor Discovery and MLD** (ICMPv6 types 130–143) between the netns and its veth are passed to the kernel, like ARP, rather than looked up. `add` turns ARP/ND off on the netns side (step 3 of [Per-ENI isolation](#per-eni-isolation-one-netns-one-veth-pair)), so this is mostly MLD — unless `--script` turns it back on.
...
3. Turn ARP/ND off on the inner end (`IFF_NOARP`). encap replaces a reply's whole Ethernet header, so the destination MAC the netns picks never reaches the wire — but with ARP on, the netns's kernel would still hold every reply until it resolved one for the next hop, and nothing answers: the root netns only answers for addresses it owns. With it off, the kernel sends straight away, to the interface's own MAC, whatever the routes look like. Nothing needs to resolve the inner end either, since decap addresses every frame it delivers to it directly. A `--script` that still pins neighbor entries keeps working; they just aren't needed anymore.
4. Disable TX checksum offload and every segmentation/GRO offload (TSO, GSO, USO, GRO, ...) on both veth ends (`vethDisabledFeatures` in [cmd/add.go](cmd/add.go)). BPF can't compute the real L4 checksum for the netns's egress traffic, so the kernel must write it before encap ever sees the packet; and every packet crossing the veth must be a single wire-sized packet, since a GSO super-packet reaching encap would be encapsulated as one oversized frame and dropped on the way out.
5. Optionally run a `--script` hook (netns name + interface name as args) so the backend/appliance side can finish its own setup before the ENI is reachable. This is where the netns's addresses and routes belong — including the route MTU its replies need (see [MTU and offloads](#mtu-and-offloads)).
6. Read both ends' MACs — only now, since the script may set the inner end's address itself, and decap writes that address into every frame it delivers — and insert `(ENI ID → outer ifindex, inner MAC, outer MAC)` into `eni_to_ifindex` and attach the shared `encap` program to the veth-outer — this is the last step, since it's what makes the ENI live.
...
| `frag_state` | encap's in-flight reply fragment tracking: a first fragment's (addresses, protocol, fragment id) → its flow's cached outer header, so the later fragments can find it. LRU — an entry only needs to outlive one datagram. Sized by `--max-frag-entries`, default 16384. |
